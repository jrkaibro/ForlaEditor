...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="GxControl">
    <xsl:choose>
      <xsl:when test="@type = 'ForlaEditor'">
        <xsl:call-template name="RenderForlaEditor"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <!-- ForlaEditor design render -->
  <!-- ///////////////////  Implement your render here  ///////////////////-->
  <xsl:template name="RenderForlaEditor">
            ...
            <img style="background-repeat: no-repeat;" padding="0">
              ...
              <xsl:attribute name ="height">
                ...
                <xsl:attribute name ="src">
                  ...
                  <xsl:text>\</xsl:text>
                  <xsl:value-of select="gxca:GetPropValue('Theme')"/>
                  <xsl:value-of select="gxca:GetPropValue('Toolbar')"/>
          ...
          <TR>
            <TD>
              <textarea Style="width:100%;height:100%">
                <xsl:text>FroalaEditor.</xsl:text>
              </textarea>
            </TD>
          </TR>
        </TBody>
